# Claude Code Project Instructions — include-dir-kotlin

## Project Overview

This is **include-dir-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`include_dir`](https://crates.io/crates/include_dir).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/include_dir/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/include_dir>
