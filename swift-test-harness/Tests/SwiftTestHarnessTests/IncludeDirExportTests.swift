...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `IncludeDir.swiftmodule/`
...
//      `import IncludeDir` above would fail at compile time.
//
//   2. The static archive `libIncludeDir.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "IncludeDir" }` and
//      `flattenPackage = "io.github.kotlinmania.includedir"` configuration in
...
//      via the `IncludeDirLibrary` product.
//
...
final class IncludeDirExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "IncludeDir swift module imported cleanly")
    }
}
